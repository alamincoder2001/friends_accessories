...
<?php
    ...
    public function materialPurchase($purchase_id = 0, $requisition_id = 0)
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
    ...
    public function addMaterialPurchase()
    {
        ...
        try {
            ...
            $purchase = array(
                ...
                "requisition_id" => isset($data->purchase->requisition_id) ? $data->purchase->requisition_id : 0,
            ...
            if (isset($data->purchase->requisition_id) && $data->purchase->requisition_id != 0) {
                $this->db->query("UPDATE tbl_material_requisition SET status = 'a' WHERE requisition_id = ? AND branch_id = ?", [$data->purchase->requisition_id, $this->brunch]);
                $this->db->query("UPDATE tbl_material_requisition_details SET status = 'a' WHERE requisition_id = ?", [$data->purchase->requisition_id]);
            }
