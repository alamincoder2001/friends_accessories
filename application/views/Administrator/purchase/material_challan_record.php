                        ...
                        <tr v-for="purchase in purchases">
                            ...
                            <td>
                                <?php if ($this->session->userdata('accountType') != 'u') { ?>
                                    <a href="" v-bind:href="`material_challan_invoice/${purchase.purchase_id}`" target="_blank"><i class="fa fa-file-text fa-2x"></i></a>
                                    <a href="" v-on:click.prevent="editChallan(purchase.purchase_id)"><i class="fa fa-pencil-square fa-2x"></i></a>
                                    <a href="" v-on:click.prevent="deleteChallan(purchase.purchase_id)"><i class="fa fa-trash fa-2x"></i></a>
                                <?php } ?>
                            </td>
                        </tr>
    ...
    new Vue({
        ...
        methods: {
            ...
            getPurchase() {
                ...
                axios.post('get_material_challan', options)
                    .then(res => {
                ...
                axios.post('/get_material_purchase', {
                    ...
                    .then(res => {
                        if (res.data.length == 0) {
                            window.location.href = '/material_challan/' + purchase_id;
                        } else {
                            alert('Purchase found. Edit not possibel')
                            return;
                        }
                    })
            },
            deleteChallan(purchase_id) {
                let conf = confirm('Are you sure?');
                if (conf == false) {
                    return;
                }
                axios.post('/delete_material_challan', {
                        purchase_id: purchase_id
                    })
                    .then(res => {
                        let r = res.data;
