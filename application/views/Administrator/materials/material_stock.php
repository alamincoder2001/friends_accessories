                ...
                <table class="table table-bordered" style="display:none" v-bind:style="{display: searchType != null ? '' : 'none'}">
                    ...
                    <tbody style="display:none;" v-bind:style="{display:stock.length > 0 ? '' : 'none'}">
                        <tr v-for="(material, sl) in stock">
                            ...
                            <td>{{ material.purchased_quantity | decimal }}</td>
                            <td>{{ material.purchased_net_weight_quantity | decimal }}</td>
                            <td>{{ material.wastage_qty | decimal }}</td>
                            <td>{{ material.production_quantity | decimal }}</td>
                            <td>{{ material.damage_quantity | decimal }}</td>
                            <td>{{ material.stock_quantity | decimal }} {{ material.unit_name}}</td>
                            <td>{{ material.stock_value | decimal }}</td>
                        </tr>
                        <tr>
                            ...
                            <th>{{ stock.reduce((prev, curr) => {
                                    return prev + parseFloat(curr.purchased_net_weight_quantity)
                                },0).toFixed(2) }}
                            </th>
                            <th>{{ stock.reduce((prev, curr) => {
                                    return prev + parseFloat(curr.wastage_qty)
                                },0).toFixed(2) }}
                            </th>
                            <th>{{ stock.reduce((prev, curr) => {
                                    return prev + parseFloat(curr.production_quantity)
                                },0).toFixed(2) }}
                            </th>
                            <th>{{ stock.reduce((prev, curr) => {
                                    return prev + parseFloat(curr.damage_quantity)
                                },0).toFixed(2) }}
                            </th>
                            <th>{{ stock.reduce((prev, curr) => {
                                    return prev + parseFloat(curr.stock_quantity)
                                },0).toFixed(2) }}
                            </th>
                            <th>{{ stock.reduce((prev, curr) => {
        ...
        methods: {
            ...
            getMaterialStock() {
                ...
                axios.post(url, parameters).then(res => {
                    if (this.searchType == 'current') {
                        this.stock = res.data.filter((pro) => pro.stock_quantity != 0).map(p => {
                            if (p.purchased_net_weight_quantity == 0) {
                                p.wastage_qty = 0
                                return p;
                            } else {
                                return p;
